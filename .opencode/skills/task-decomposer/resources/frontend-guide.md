...
Step 4 加载。按 `LC-FE-001` 选框架。这里给**项目专属的设计决策锚点**，标准框架知识不在此重复（那是教程，评审不抓）。

## 设计决策锚点（详设必须回答）

| 决策 | 锚点 | 不通过信号 |
|------|------|-----------|
| 状态管理 | 全局 store 只放跨组件共享态，本地态用 ref/useState | 全部状态塞 store / 组件间 props 深层穿透 |
| 数据流 | 请求统一走 `api/` 层 + 拦截器（token/错误/401 刷新） | 组件内裸 fetch、错误就地吞 |
| 组件边界 | 同 UI 模式出现 2+ 次必须抽共享组件 | views 级复制粘贴 |
| 路由 | 懒加载 + 路由守卫鉴权 | 整包一次性加载、守卫缺失 |
| 样式 | 全部走 CSS 变量（variables.css），禁内联样式 | 硬编码色值/px、`:style="{}"` |
| 表单 | 统一校验方案（rules/validate），错误就地展示 | 提交时才报错、无必填校验 |
