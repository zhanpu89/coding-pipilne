...
| **L1** | 用户选择继续某任务 | `get_summary` 完整摘要 | ~500–1000 |
| **L2** | 遇到复杂问题需历史参考 | `search_summaries` 3条 | ~1500–3000 |
| **L3** | 用户明确要求完整回顾 | `list_recent` + 逐条详情 | ~3000+ |
...
| `get_summary` | 获取单条完整摘要 | 记忆加载 / 记忆检索 |
| `list_recent` | 列出最近会话（标题+状态） | 记忆加载 |
| `save_summary` | 保存新摘要到记忆库 | 记忆保存 |
| `update_summary` | 更新摘要状态或内容 | 记忆丰富 |
| `add_decision` | 记录关键技术决策 | 记忆丰富 / 记忆保存 |
| `search_summaries` | 三级检索（语义→全文→关键词） | 记忆检索 |
