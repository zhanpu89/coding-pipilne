...
| 文件 | 加载时机 | 释放时机 | 行数 |
|------|---------|---------|------|
| `resources/interview-framework.md` | 标准模式 Step 1 前——访谈与提问框架（⚡快速模式不加载） | Step 3 生成 PRD 前 | 162 |
...
| 模式 | 触发条件 | 执行步骤 |
|------|---------|---------|
| **🔄 标准模式（交互式）** | 用户描述模糊，或任务中未包含结构化需求摘要 | Step 0 → Step 1 → Step 2 → Step 3 |
| **⚡ 快速模式（生成式）** | 任务中包含结构化需求摘要（如 `_requirements_summary.md`），或明确标注"需求已收集完毕" | → **直接跳转到 Step 3** |

> 快速模式由 pipeline-orchestrator 编排器在需求访谈完成后触发，避免 subagent 无法使用 `question` 工具的问题。
...
**检测输入模式：**
- 任务描述中包含"需求已收集完毕"或"跳过 Steps 0-2" → **⚡ 快速模式**：直接跳转到 Step 3
- 否则 → **🔄 标准模式**：继续执行 Step 0-2

#### 🔄 标准模式下的 Step 0：端类型识别与锁定（第一轮必执行）
...
仅在标准模式下执行。加载 `resources/interview-framework.md`。用 5W1H 框架复述理解+识别缺口+提 5~8 个问题。用户描述极度模糊时先锁端类型再问业务目标。

#### 🔄 标准模式下的 Step 2：多轮澄清

仅在标准模式下执行。每轮 5~8 个问题（继续使用 `resources/interview-framework.md` 中的框架）：边界澄清、用户与场景、异常边界、冲突处理、KANO 优先级、端专属问题。连续两轮用户无法回答则记录"待补充"继续。

> 释放提示：Step 2 结束时 `interview-framework.md` 不再需要，可释放上下文中不再保留。

### Step 3：PRD 生成

**⚡ 快速模式**：直接加载需求摘要，按端类型生成 PRD。
**🔄 标准模式**：范围确认清单全部满足后执行。

释放 `interview-framework.md`（如仍占用）。按 Step 0 锁定的端类型清单，按需加载以下文件：
