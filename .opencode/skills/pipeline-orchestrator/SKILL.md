...
**Step 0（执行以下 5 个子步骤，不可跳过）：**

```
① ai_memory_init_session()
   
② 检索历史经验（多角度搜索）
   ▸ 角度 A — 模块名
     调用: search_summaries(query=任务涉及的模块名, module=模块名, limit=3)
     如不确定模块名则跳过此角度。
   ▸ 角度 B — 技术关键词
     调用: search_summaries(query=从任务描述中提取的 1~2 个具体技术词, use_fts=true, limit=3)
     提取原则：优先取函数名/类名/框架名/库名/报错信息；不要提取通用词如"修复""新增"。
   ▸ 角度 C — 自然语言描述
     调用: search_summaries(query=任务核心问题(20 字以内), use_vector=true, limit=3)
     注意：use_vector=true 是语义检索，不需要关键词重叠。把任务的核心矛盾说出来即可。
   ▸ 如果以上角度的相关结果合计不足 3 条
     补充调用: list_recent_sessions(limit=5) — 取最近任务兜底
...
③ 相关性判断（对每条返回结果执行）
   P-高：模块相同 + 问题类型相似 → 必须注入
   P-中：模块不同但技术模式相同 → 有条件注入
   P-低：仅时间接近或关键词偶然匹配 → 跳过
   
④ 产出 _MEMORY_CACHE.md [历史经验] 段
   ```
   【历史经验参考（来自项目记忆）】
   搜索角度 A（模块名: xxx）：
     - [{相关度}] {session title} — {summary 第一句}
   搜索角度 B（技术关键词: xxx）：
     - [{相关度}] {session title} — {summary 第一句}
   搜索角度 C（自然语言: xxx）：
     - [{相关度}] {session title} — {summary 第一句}
   注入理由：
     ...
     - {结果2} 对本次任务有帮助：{具体说明}
   ```
   【注入前最后筛子】每条注入结果必须能回答"对下一步 Phase 有什么具体帮助？"。答不出就放弃注入。
   
⑤ 扫描项目结构 + 分析任务影响域 → 自动匹配流程强度
```
