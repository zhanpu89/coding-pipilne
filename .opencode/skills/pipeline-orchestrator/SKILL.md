...
每个 Phase 调用 `task(subagent_type='{agent}')` → `bash .opencode/scripts/check-{phase}.sh`（失败 ≤3 次）→ `ai_memory_add_decision()`。
...
每 Phase 完成后丢弃该 Phase 的中间输出（task 返回值、门禁检查结果、用户确认内容）。只保留：当前 Phase 编号、产出物路径清单、关键决策。`ai_memory_add_decision()` 在每个产出 Phase 末尾调用，不做 Phase 间上下文传递。
