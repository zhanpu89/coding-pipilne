...
**读决策（OUT）：** 重启或 Step 0 时，编排器按以下优先级检索：

| 优先级 | 检索方式 | 参数 | 目标 |
|--------|---------|------|------|
| P0 | `search_summaries(query=项目名, module=当前模块, limit=5)` | 精确项目 + 模块 | 同模块历史经验 |
| P1 | `search_summaries(query=任务关键词, tags=领域标签, use_fts=true, limit=5)` | 任务关键词 + 标签 | 同类问题的解决方案 |
| P2 | `search_summaries(query=任务描述, use_vector=true, limit=5)` | 语义检索 | 模糊匹配的相似案例 |
| P3 | `list_recent_sessions(limit=5)` | 最近任务 | 最新经验兜底 |

每条结果前置判断：**是否与本任务相关？** 不相关的即使检索到了也丢弃，不喂给下一 Phase。

---
**Step 0：** `ai_memory_init_session()` → 按"读决策"规则检索（P0→P1→P2→P3）→ 结果写入 `_MEMORY_CACHE.md` [历史经验] 段 → 扫描项目结构 + 分析任务影响域 → 自动匹配流程强度。
