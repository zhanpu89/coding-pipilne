...
> - **T1 回归范围**：`>>SCOPE: modules=` 中显式加入 `SIDE-EFFECT` 涉及模块（受影响模块即使不是原始 Bug 模块也要定向回归）
> - code-developer 没输出 `>>SIDE-EFFECT:` 标记时，编排器用 `git diff` 推断（改动的函数 → 其调用方），并把推断结果写入 `_MEMORY_CACHE.md` 供 P5b/T1 使用
>
