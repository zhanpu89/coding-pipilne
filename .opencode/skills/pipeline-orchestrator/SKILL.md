...
> 🔗 特征交互图见 `resources/interaction-map.md` — OODA/置信度/反馈/恢复/自进化五能力闭环

Step 0 选择的流程强度（🐛/🟢-light/🟢/🟡/🔴）是**起始计划，不是最终计划**。编排器在每 Phase 的后置 OODA **Decide** 步骤中评估是否需要调整后续阶段序列——就像人类工程师边做边调整工作计划。
...
| 信号 | 动作 |
|------|------|
| 设计评审发现遗漏约束或隐藏依赖 | P3a(简设) → P3a(详设) → 增加 P4a/b(DDL) |
| 编码中发现现有文档缺失关键契约 | 插入 P1b/c 补 PRD 或 P2a/b 补架构 |
| 测试设计时发现覆盖缺口远超预期 | P6c(存量) → P6a→P6b→P6c(完整流程) |
| Spike 确认技术栈比预想复杂 | 增加 P2a/b(架构) 补充技术决策 |

**降档条件（可减少 Phase）：**
| 信号 | 动作 |
|------|------|
| 设计验证通过且改动极简单（只改文案级） | 🟢 跳过 P5b 评审，编排器自检即可 |
| DDL 评审确认无风险 | 🟡 合并 P5a/P5b 一次提交 |
| 存量测试充分覆盖改动 | P6c 仅跑存量，不生成新用例 |

**执行方式：** 在后置 OODA 的 Decide 中明确写出计划调整并更新 `_MEMORY_CACHE.md` [Phase 上下文] 的总 Phase 数和进度：
```
【OODA Phase 3a 后置】
...
Decide: 设计发现 2 个隐藏约束 → 升档：追加 P4a/b(DDL) → Phase 序列调整为 5 步
```

> 调整后更新 `_MEMORY_CACHE.md` [Phase 上下文] 的"总 Phase 数"和进度百分比。不需要通知用户——高级工程师自己调整工作计划不需要每次汇报。

### 管道中段反思（Vibe Check）

**问题：** 编排器连续执行 Phase 时可能忽略早期决策的信号漂移，或陷入局部最优。人类工程师会定期抬头看看方向。

**触发时机：** 以下任一条件满足时触发：
- 累计完成 3 个 Phase（首次反思）
- 自适应恢复矩阵连续 2 次触发（无论类型）
- 跨 Phase 反馈 🔴 阻断型被触发后
- 动态阶段调整发生升档后

**反思内容：** 停一停，思考 4 个问题：
1. **轨迹检查：** 当前进展是否符合 Step 0 的预期？和原始任务描述还有偏差吗？
2. **决策验证：** 翻看前 3 个 Phase 的 `ai_memory_memory_add_decision` 记录，有没有后悔的决策？
3. **置信度更新：** 置信度需要调整吗？（🟢→🟡 或 🟡→🟢）
4. **风险预警：** 有没有早期警告信号被忽略了？

**反思输出：** 追加到 `_MEMORY_CACHE.md`：
```
【中段反思 — Phase {当前编号} 完成时】
轨迹: 🟢 按计划 / 🟡 有偏差但可控 / 🔴 需要调整计划
决策验证: {前序决策是否仍然有效？有需要修正的吗？}
置信度更新: {不变 / 升级 / 降级}
调整: {计划变更或确认继续}
```

> 反思不中断流程。即使轨迹是 🔴 也只是调整计划而非暂停——除非是 🔴 阻断型跨 Phase 反馈才触发暂停。
...
失败时加载 `resources/recovery-matrix.md` 按类型选择恢复策略（🅰崩溃/🅱评审/🅲质量/🅳死循环）。执行规则中的 "失败按自适应恢复矩阵处理" 即指此。
