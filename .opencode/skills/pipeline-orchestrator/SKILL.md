...
每个 Phase 调用 `task(subagent_type='{agent}')` → `bash .opencode/scripts/check-{phase}.sh`（失败 ≤3 次）→ `ai_memory_add_decision()`（遵循"决策存取质量规则"的写决策模板和 taxonomy）。首个产出 Phase 额外调 `ai_memory_save_summary(status=in_progress, tags=模块名)`，后续调 `update_summary`。
...
Step 0 统一检索一次，结果写入 `_MEMORY_CACHE.md`。每个 Phase 重启后编排器从 `_MEMORY_CACHE.md` 构建上下文：

| 来源 | 内容 | 写入时机 |
|------|------|---------|
| `_MEMORY_CACHE.md` [历史经验] | 本次运行前的历史经验（跨 pipeline 复用） | Step 0（只写一次） |
| `_MEMORY_CACHE.md` [Phase 上下文] | 当前 Phase 编号、产出物路径、关键决策 | 每 Phase 末尾重写 |
| `ai_memory` 记忆库 | 完整追溯，供读 `get_summary_by_id` 使用 | 持久层，不写入缓存 |
...
**解决方案 — Phase 终了协议：** 每个 Phase 执行完毕后，编排器执行 4 步断点，使下一 Phase 在干净上下文中重启：

| 步骤 | 动作 | 说明 |
|------|------|------|
| ① 持久化 | `ai_memory_add_decision()` + `update_summary()` | 记录当前 Phase 的关键决策到记忆库 |
| ② 写缓存 | 重写 `_MEMORY_CACHE.md`，覆盖 [Phase 上下文] 段 | 只保留下一 Phase 需要的：编号、产出路径、3~5 条决策 |
| ③ 终止 | 输出 Phase 完成摘要 + 终止信号 | 格式见下文 |
| ④ 重启 | 用户回复"继续"后，读 `_MEMORY_CACHE.md` 构建下一 Phase 上下文 | 依赖缓存，不依赖历史对话 |
...
| 后续每个 Phase 末尾 | Phase 终了协议 ①~③ | 持久化 + 写缓存 + 终止 |
| 重启后（用户输入"继续"） | 读 `_MEMORY_CACHE.md` 构建 prompt | 不读取历史对话 |
| Step 7 | `ai_memory_update_summary(status=completed)` | 标记完成 + 清理临时文件 |

**关键约束：** Phase 终止后，编排器不能假设下一轮能访问本轮的对话历史。所有必须传递的信息必须通过 `_MEMORY_CACHE.md` 和 `ai_memory` 持久化。下一 Phase 的首条 prompt 只读 `_MEMORY_CACHE.md` + `ai_memory_get_summary_by_id()`，不再依赖历史上下文。

### 决策存取质量规则

ai_memory 是 stateless 模式的唯一长效记忆层。**存入质量决定取出价值。**

**写决策（IN）：** 每条 `ai_memory_add_decision()` 必须包含：
...
AI 检索的最大问题是"猜一个关键词就搜，搜到什么用什么"。正确方式是像人一样**多角度探索 + 迭代 + 相关性判断**。

**核心方法：一轮探索包含 3 个步骤，不足则再开一轮。**

一轮探索：

```
① 提取 2~3 个不同的搜索角度
   角度 A：模块名（如 "comment"）→ search_summaries(query="comment", module="comment", limit=3)
   角度 B：技术关键词（如 "asyncio.Lock" "JSON file"）→ search_summaries(query=具体技术词, use_fts=true, limit=3)
   角度 C：任务核心动词+名词（如 "并发写入冲突" "编码异常"）→ search_summaries(query=自然语言描述, use_vector=true, limit=3)
   
② 判断每条结果的相关度
   相关度等级 | 判断依据 | 处理
   P-高 | 模块相同 + 问题类型相似 | 必须注入上下文
   P-中 | 模块不同但技术栈/问题模式相同 | 选择性注入（影响域分析时参考）
   P-低 | 仅时间接近或关键词偶然匹配 | 不注入，跳过
   
③ 统计相关结果数
   ≥3 条 P-高 → 停止搜索，注入上下文
   1~2 条 P-高 → 开始第二轮搜索（换角度 B）
   0 条 P-高 → 开始第二轮搜索（换角度 C）
```
...
| 你要找的 | 使用的参数 | 原因 |
|---------|-----------|------|
| 精确报错信息、函数名、类名 | `use_fts=true` | FTS5 精确匹配，不含糊 |
| 同模块的历史经验 | `module=模块名` + `query=模块名` | 模块名是最强过滤条件 |
| 标签明确的已知问题 | `tags=标签` + `use_fts=true` | 标签是人工标注的精确分类 |
| 自然语言描述的模糊问题 | `use_vector=true` | 语义匹配，不需要关键词重叠 |
| 以上都不确定 | 先 `use_fts=true`，结果不足 3 条时重复一遍 `use_vector=true` | 精确优先，模糊兜底 |

**冷启动规则（新项目无历史记录时）：**
1. 跳过 P0（项目名精确搜索返回空）
2. 跳过错失 P1（无标签积累）
3. 直接用 P2 向量搜索：`search_summaries(query=任务描述, use_vector=true, limit=5)`
4. 用 P3 兜底：`list_recent_sessions(limit=5)` — 从其他项目的经验中找模式相似的案例

**注入前的最后一道筛子：** 对上一步确认的 P-高 和 P-中 结果，显式判断"这条经验对我的下一步 Phase 有什么具体帮助？"。说不出具体帮助的，即使是 P-高 也放弃注入，宁可少不可滥。

---
**Step 0：** `ai_memory_init_session()` → 按"读决策"检索策略执行探索（多角度搜索 + 相关性判断，直到获得 ≥3 条 P-高 或用完所有角度）→ 结果写入 `_MEMORY_CACHE.md` [历史经验] 段 → 扫描项目结构 + 分析任务影响域 → 自动匹配流程强度。
...
> 产出 Phase 结束后结束 subagent。门禁失败 ≤3 次。每 Phase 执行 Phase 终了协议（持久化 + 写缓存 + 终止），下一 Phase 重启后从 `_MEMORY_CACHE.md` 构建上下文。
>
...
**Step 7（最终清理）：** `ai_memory_update_summary(status=completed)` 归档 → 删除 `_MEMORY_CACHE.md`、`_contract_check.md` 等临时文件 → 输出 ✅ **Pipeline 完成** + 产出物汇总。

> 注意：Step 7 是全局最终清理。每个 Phase 的局部清理由 Phase 终了协议的 ②（写缓存，覆盖旧内容）+ ③（终止，丢弃会话历史）完成。
