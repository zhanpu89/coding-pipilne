...
**ai-memory（记忆桥梁）贯穿全流程**：每个阶段通过 ai_memory_* 工具持久化决策和摘要。
...
2. 用 `task(subagent_type='general', prompt="<完整指令>")` 启动独立的 subagent
...
| 步骤 | 保持加载 | 释放 |
|------|---------|------|
| Step 0 | pipeline-definition.md | 完成后释放 |
| Step 1-6 | check-*.sh 脚本路径 | 全局使用 |
| Step 7 | 汇总清单 | 完成后|

## 合并原则

无拆分。所有阶段定义在 `pipeline-definition.md`，SKILL.md 聚焦流程编排（task+bash 模式）。

## 参考文件

| 文件 | 行数 | 步骤 | 加载时机 | 释放时机 |
|------|------|------|---------|---------|
| `resources/pipeline-definition.md` | 130 | Step 0 | Step 0 开始时 | Pipeline 完成后 |
| `resources/validation-scripts.md`（内联） | — | Step 1-6 | 各步需要时 | 使用后 |
...
1. 加载 `resources/pipeline-definition.md`（用 `read` 工具）
2. **记忆操作**：`ai_memory_init_session(project_name=<项目名>)` — 若失败则继续（非阻断）
3. 读取 `doc/pipeline/_PROGRESS.md`（若存在）：
...
```
task(subagent_type='general', prompt="<call_prd_writer 返回的完整指令>")
```
...
```
task(subagent_type='general', prompt="<call_review_expert 返回的完整指令>")
```
...
```
task(subagent_type='general', prompt="<call_system_architect 返回的完整指令>")
```

验证：`bash .opencode/scripts/check-arch.sh`

**记忆**：`ai_memory_add_decision(session_id, "arch_complete", "架构文档已生成: doc/arch/ + tech-stack.json")`
...
**记忆**：`ai_memory_add_decision(session_id, "arch_review", "架构评审结论: ${结论}")`
...
```
task(subagent_type='general', prompt="<call_task_decomposer 返回的完整指令>")
```

验证：`bash .opencode/scripts/check-detailed.sh`

**记忆**：`ai_memory_add_decision(session_id, "detailed_complete", "详设文档已生成: doc/detailed/")`
...
**记忆**：`ai_memory_add_decision(session_id, "detailed_review", "详设评审结论: ${结论}")`
...
```
task(subagent_type='general', prompt="<call_dba_designer 返回的完整指令>")
```

验证：`bash .opencode/scripts/check-db.sh`

**记忆**：`ai_memory_add_decision(session_id, "db_design_complete", "DDL 已生成: doc/db/")`
...
```
task(subagent_type='general', prompt="<call_code_developer 返回的完整指令>")
```

验证：`bash .opencode/scripts/check-code.sh`（失败重跑 ≤3次）

**记忆**：`ai_memory_add_decision(session_id, "coding_complete", "代码已实现: src/")`
...
```
task(subagent_type='general', prompt="<call_code_reviewer 返回的完整指令>")
```
...
**记忆**：`ai_memory_add_decision(session_id, "code_review", "代码评审结论: ${结论}")`
...
```
task(subagent_type='general', prompt="<call_tester 返回的完整指令>")
```
...
**记忆**：`ai_memory_add_decision(session_id, "test_case_design", "测试用例已生成: doc/tester/")`
...
**记忆**：`ai_memory_add_decision(session_id, "test_case_review", "测试用例评审结论: ${结论}")`
...
**记忆**：`ai_memory_save_summary(session_id, "Pipeline 完成", summary_content="完整产出物清单...", status="completed", tags="pipeline,full-cycle")`
