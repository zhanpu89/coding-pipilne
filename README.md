...
- **记忆注入** — 每个 Phase 启动前自动检索历史经验，避免重复踩坑
...
| Code Developer | `code-developer` | `call_code_developer` | 编码实现（含精确到位、doc sync） | `src/` |
...
每个 Phase 启动 subagent **前**，编排器先 `search_summaries` 检索项目历史经验，包装为 `【历史经验参考】` 注入 prompt。避免之前犯过的错在后续阶段重复出现。
...
**Q: ai-memory 起什么作用？**
A: 它不仅是记忆持久化工具，更是 pipeline 的**经验引擎**。每个 Phase 启动前，编排器自动 `search_summaries` 检索历史经验注入 subagent prompt，让后续阶段从过去的决策和踩坑中学习。作为"经验脊椎"贯穿所有阶段。
