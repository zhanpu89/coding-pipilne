...
**Q: ai-memory 起什么作用？**
A: 它通过 MCP 工具（`ai_memory_init_session`/`ai_memory_add_decision`/`ai_memory_save_summary`/`ai_memory_search_summaries`）将全流程的关键决策和阶段产出持久化，确保长周期开发中上下文不丢失。作为"记忆脊椎"贯穿所有阶段。
